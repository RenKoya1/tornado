...
function generateMerkleProof(depositEventsJsonPath, leafIndex) {
  const depositEventsJson = JSON.parse(
    fs.readFileSync(__dirname + "/../" + depositEventsJsonPath)
  );
  const leaves = depositEventsJson.commitments.map((commitment) =>
...
async function generateInput({
  ...
  const { root, pathElements, pathIndices } = generateMerkleProof(
    depositEventsJsonPath,
    leafIndex
  );
...
async function main() {
  program
    .command("gen-deposit")
    .description(
      "Generate a deposit object (nullifier, secret, preimage, and commitment) and print it to stdout as JSON data"
    )
    .action(() => {
  ...
  program
    .command(
      "gen-input <deposit_events_json_path> <nullifier> <nullifier_hash> <secret> <leaf_index> <recipient>"
    )
    .description(
      "Generate the witness of specified deposit commitment and deposit events and print it to stdout as JSON data"
    )
    .action(
      async (
        depositEventsJsonPath,
